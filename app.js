...
// brevi. Non esiste un numero fisso di righe da scartare (varia da pagina a
// pagina, a volte e' zero), quindi il filtro si basa sulla lunghezza del testo
...
function parseLastStickerChecklist(html) {
  ...
  if (!table) throw new Error("Nessuna tabella trovata nella pagina");
  ...
  rows.forEach((row) => {
    ...
    const titolo = cols[1].textContent.trim();
    const sezione = cols[2].textContent.trim();
    if (!numero || !titolo || !sezione) return;
    if (numero.toLowerCase() === "no." || titolo.toLowerCase() === "titolo") return;
    if (titolo.length > LUNGHEZZA_MASSIMA_PLAUSIBILE || sezione.length > LUNGHEZZA_MASSIMA_PLAUSIBILE) {
      scartate.push({ name: titolo, team: sezione, numero });
      return;
    }
    data.push({ name: titolo, team: sezione, stkn: "STK N", numero });
  });

  if (data.length === 0) throw new Error("Nessun dato estratto dalla tabella");
...
document.getElementById("scrapeBtn").addEventListener("click", async () => {
  ...
  try {
    ...
    if (scartate.length) {
      ...
      scartate.forEach((r) => log("scrapeLog", `  (scartata) ${r.name.slice(0, 40)}... | ${r.numero}`));
    }
    log("scrapeLog", `Sticker estratti (validi): ${records.length}`);
    log("scrapeLog", "\nPrime 5 righe:");
    records.slice(0, 5).forEach((r) => log("scrapeLog", `  ${r.name} | ${r.team} | STK N | ${r.numero}`));
    log("scrapeLog", "\nNessuna classificazione automatica per tipo: le collezioni non seguono uno "
      + "schema di numerazione uniforme, quindi qualunque etichetta automatica rischierebbe di essere "
      + "sbagliata in modo silenzioso. Scarica il file e raggruppa/filtra tu in Excel.");

    excelRecords = records;
    document.getElementById("excelStatus").textContent = `Checklist generata da LastSticker: ${records.length} righe`;
    document.getElementById("runStep2Btn").disabled = false;

    // Prepara anche il download come .xlsx, per riuso/verifica manuale
    // Colonna E: stringa completa gia' composta nello stesso formato scritto
    // dallo Step 2 nell'IPTC (utile come riferimento per correzioni manuali)
    const wsData = [["Titolo", "Sezione", "STK N", "Numero", "Stringa completa"]]
      .concat(records.map((r) => [r.name, r.team, r.stkn, r.numero, `${r.name} - ${r.team} ${r.stkn} ${r.numero}`]));
    ...
    const a = document.getElementById("downloadChecklistLink");
    a.href = dlUrl;
...
document.getElementById("runStep2Btn").addEventListener("click", async () => {
  ...
  const assegnati = {}; // numero sticker -> [nomi file] (per rilevare doppi matching)
  ...
  for (let i = 0; i < total; i++) {
    ...
    try {
      ...
      if (rec) {
        ...
        if (!assegnati[rec.numero]) assegnati[rec.numero] = [];
        assegnati[rec.numero].push({ file: name, desc });
      } else {
  ...
  // Controllo doppi: stesso codice STK assegnato a piu' file diverso -> probabile
  // matching sbagliato tra OCR e checklist (es. due giocatori diversi abbinati
  // per errore allo stesso codice).
  const duplicati = Object.entries(assegnati).filter(([, files]) => files.length > 1);
  const duplicatiLines = [];
  if (duplicati.length > 0) {
    log("step2Log", `\nATTENZIONE: ${duplicati.length} codici STK assegnati a piu' di un file (controllare a mano):`);
    duplicati.forEach(([numero, files]) => {
      log("step2Log", `  STK N ${numero} assegnato a ${files.length} file:`);
      duplicatiLines.push(`STK N ${numero} assegnato a ${files.length} file:`);
      files.forEach((f) => {
        log("step2Log", `    - ${f.file} -> ${f.desc}`);
        duplicatiLines.push(`    - ${f.file} -> ${f.desc}`);
      });
    });
  } else {
    log("step2Log", "\nNessun codice STK duplicato tra i file processati.");
  }
  ...
  if (duplicatiLines.length > 0) {
    const blob = new Blob([duplicatiLines.join("\n")], { type: "text/plain" });
    const url = URL.createObjectURL(blob);
    const a = document.getElementById("duplicatesLink");
    a.href = url;
    a.download = "REPORT_DUPLICATI.txt";
    a.style.display = "inline-block";
  }
  btn.disabled = false;
});
