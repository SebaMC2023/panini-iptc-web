...
const RIGHE_INIZIALI_DA_SCARTARE = 2; // stesso fix applicato a scrape_checklist.py
...
function parseLastStickerChecklist(html) {
  ...
  let scartate = [];
  if (data.length > RIGHE_INIZIALI_DA_SCARTARE) {
    scartate = data.slice(0, RIGHE_INIZIALI_DA_SCARTARE);
  }
  const clean = data.slice(RIGHE_INIZIALI_DA_SCARTARE);
  return { records: clean, scartate };
}
...
document.getElementById("scrapeBtn").addEventListener("click", async () => {
  ...
  try {
    ...
    if (scartate.length) {
      log("scrapeLog", `Scartate le prime ${scartate.length} righe non valide:`);
      scartate.forEach((r) => log("scrapeLog", `  (scartata) ${r.name} | ${r.team} | ${r.numero}`));
    }
    ...
    const wsData = [["Titolo", "Sezione", "STK N", "Numero"]]
      .concat(records.map((r) => [r.name, r.team, r.stkn, r.numero]));
